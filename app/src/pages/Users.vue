  ...
  <v-layout align-top>
    <v-flex xs12 md10 offset-md1 top>
      ...
      <CreateUnregistredUserDialog
        v-bind:showCreateDialog="showCreateDialog"
        v-on:userCreated=""></CreateUnregistredUserDialog>
    </v-flex>
  </v-layout>
</template>
...
<script>
  ...
  export default {
    name: "Users",
    components: {CreateUnregistredUserDialog, GroupsSelectDialog},
    data: function () {
      return {
        dialog: false,
    ...
    methods: {
      ...
      dataLoaded(res) {
        ...
        for (let i = 0; i < res.data.data.length; i++) {
          ...
          this.users.push(new User({id: userObj.id, email: userObj.email, firstName: userObj.firstName, familyName: userObj.familyName, active: userObj.active, groupId: userObj.groupId, roleNames: userObj.roleNames, registered: userObj.registered}))
        }
